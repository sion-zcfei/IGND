...
class DecoderRNN(nn.Module):
  def __init__(self, vocab_size, embed_size, hidden_size, *, rnn_type='lstm', enc_attn=True, dec_attn=True,
               ...
               in_drop: float=0, rnn_drop: float=0, out_drop: float=0, enc_hidden_size=None, config=None, device=None):
    ...
    if enc_attn:
      ...
      if config['f_tag']:
        self.tag_embed = nn.Embedding(4, config['tag_embed_dim'], padding_idx=0)
        self.tag_linear = nn.Linear(config['tag_embed_dim'], config['edge_embed_dim'], bias=False)
        self.enc_attn_fn = Attention(self.hidden_size, 2 * self.hidden_size, self.hidden_size + config['edge_embed_dim'],
                                     attn_type='add',config=config)
      else:
          self.enc_attn_fn = Attention(self.hidden_size, 2 * self.hidden_size, self.hidden_size,
                                       attn_type='add', config=config)
  ...
  def forward(self, embedded, rnn_state, encoder_hiddens=None, decoder_hiddens=None, coverage_vector=None, *,
              ...
              prev_enc_context=None, graph_decoder=None, graph_input=None):
    """
    ...
    if graph_input != None:
        tag, edge_vec, adj = graph_input
        #tag = copy.deepcopy(input_tag)
    ...
    if self.enc_attn or self.pointer:
      ...
      if self.enc_attn_cover and coverage_vector is not None:
        ...
        addition_vec = None
      if graph_decoder != None:
          tag_embedding = self.tag_embed(tag)
          dnode_states = self.tag_linear(tag_embedding)
          node_hiddens = graph_decoder(dnode_states, tag, edge_vec, adj)
          node_hiddens = torch.cat([encoder_hiddens, node_hiddens], dim=-1)
          enc_energy = self.enc_attn_fn(hidden, node_hiddens.transpose(0, 1).contiguous(), \
