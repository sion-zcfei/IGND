...
from ..layers.graphs import GraphNN, RelaGraph
...
class Graph2Seq(nn.Module):
  ...
  def __init__(self, config, word_embedding, word_vocab):
    ...
    if self.f_bio:
      self.bio_embed = nn.Embedding(4, config['bio_embed_dim'], padding_idx=0)
      enc_input_dim += config['bio_embed_dim']
    if self.f_pos:
    ...
    if config['graph_decoder']:
      self.graph_decoder = RelaGraph(config)
    else:
    ...
    self.decoder = DecoderRNN(self.vocab_size, config['word_embed_dim'], dec_hidden_size, rnn_type=self.rnn_type,
                              ...
                              out_drop=config['dec_out_dropout'], enc_hidden_size=enc_hidden_size,
  ...
  def forward(self, ex, target_tensor=None, criterion=None, criterion_reduction=True, criterion_nll_only=False, \
    ...
    if saved_out:  # reuse encoder states of a previous run
      ...
      if self.f_bio:
        bio_features = self.bio_embed(ex['context_bio'])
      ...
      #print(encoder_embedded.size(), case_features.size(), bio_features.size(), pos_features.size(),
            #ner_features.size(), ctx_aware_ans_emb.size())
    ...
    for di in range(target_length):
      ...
      decoder_output, decoder_state, dec_enc_attn, dec_prob_ptr, enc_context = \
          self.decoder(decoder_embedded, decoder_state, encoder_outputs,
                       ...
                       graph_input=(input_tag, edge_vec, (input_graphs['node2edge'], input_graphs['edge2node'])))
      ...
      if self.f_tag:
        new_tag = input_tag.clone()
        for i in range(batch_size):
          if top_idx[i] >= self.vocab_size:
            new_tag[i][top_idx[i]-self.vocab_size] = 3
        input_tag = new_tag
